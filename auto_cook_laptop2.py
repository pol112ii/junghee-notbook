...
안전장치:
  - 창 열림은 OS 창 제목('음식만들기')으로 확인 (픽셀 색은 보조)
  - 재료 넣기 전 / 시작 버튼 직전에 창이 진짜 열려있는지 확인
  - 드래그마다 슬롯에 실제로 들어갔는지 확인, 시작 전 5칸 전부 확인
  - 오류가 나면 자가점검 후 재시도, 연속 실패 시 1분 뒤 자동 재시작
"""
...
# 픽셀 방식 기준 — 너무 느슨하면 게임 배경(초록)을 창으로 오판하고, 너무 조이면
# 진짜 열린 창도 탈락시킴. 틀리면 실패 시 출력되는 [진단] 측정값을 보고
# GAUGE_BG_RGB와 이 값들을 맞출 것.
...
def bag_open():
    """가방(아이템) 창이 보이는지 — '아이템' 제목의 창이 있는지로 확인.

    pygetwindow가 없으면 확인할 방법이 없어서 열려있다고 가정함.
    ...
    try:
        for w in _gw.getAllWindows():
            if w.title == "아이템" and w.width > 0 and w.visible:
                return True
        return False
    except Exception:
        return True
...
def fill_slots(sct, templates):
    ...
    드래그할 때마다 슬롯에 실제로 들어갔는지 확인하고, 안 들어갔으면 같은
    재료의 다른 칸으로 재시도. 마지막에 슬롯 전체가 채워졌는지 최종 확인.
    """
    ...
    if all(slot_filled(sct, i) for i in range(len(plan))):
        print("[중단] 드래그 전인데 슬롯이 전부 차 있는 것으로 보임 — "
    ...
    # 드래그 + 실제로 들어갔는지 확인. 안 들어가면 다른 칸으로 재시도하고,
    # 시도할 칸이 다 떨어지면(=캐시가 낡아 빈 칸을 집은 것) 즉시 재스캔 1번.
    used = {}
    rescanned = False
    for slot, name in enumerate(plan):
        if slot_filled(sct, slot):     # (재스캔 흐름에서) 이미 들어간 슬롯은 건너뜀
            continue
        placed = False
        while running and alive and not placed:
            ...
            if name not in found or idx >= len(found[name]):
                if rescanned:
                    break              # 재스캔까지 했는데도 없음 → 진짜 부족
                print(f"'{name}' 시도할 칸이 다 떨어짐 → 캐시가 낡은 것으로 보고 즉시 재스캔")
                found, min_diffs = _scan_with_retries(sct, templates)
                _scan_cache, _rounds_since_scan = found, 0
                used = {}              # 새 스캔은 현재 인벤토리 기준이라 처음부터 셈
                rescanned = True
                continue
            src = found[name][idx]
            used[name] = idx + 1
            print(f"'{name}' → 슬롯 {slot+1} 드래그")
            human_drag(src, (SLOT1_CENTER[0] + slot * SLOT_PITCH_X, SLOT1_CENTER[1]))
        ...
        if not (running and alive):
            return False
        if not placed:
            print(f"[중단] 재료 '{name}' 를 슬롯 {slot+1}에 넣지 못함")
            try:
                save_debug_scan(sct)
            except Exception:
                pass
    ...
    # 시작 전 최종 확인 — 요청한 재료 개수만큼 슬롯이 전부 실제로 채워졌는지
    empty = [i + 1 for i in range(len(plan)) if not slot_filled(sct, i)]
    if empty:
        print(f"[중단] 재료 넣기 후 확인 — 슬롯 {empty} 이 비어 있음")
        _scan_cache = None
        return False
